...
public class PerformanceCreateDialog extends CommonMonitorNumericDialog {
	...
	protected void customizeDialog(Composite parent) {
		...
		m_comboCollectorItem.getCombo().addModifyListener(new ModifyListener(){
			...
			public void modifyText(ModifyEvent arg0){
				...
				if(m_comboCollectorItem.getCollectorItem() != null){
					measure.setText(CollectorItemCodeFactory.getMeasure(
		...
		if(this.monitorId == null){
			...
			try {
				...
				info = wrapper.getMonitor(this.monitorId, HinemosModuleConstant.MONITOR_PERFORMANCE);
	...
	protected void update() {
		...
		try{
			...
			item1 = Messages.getString("select.value")
					+ "("
					+ CollectorItemCodeFactory.getMeasure(this.getManagerName(),
	...
	protected MonitorInfo createInputData() {
		...
		// 内訳を収集するかのフラグ
		if(m_breakdownFlg.getSelection()){
			perfInfo.setBreakdownFlg(new Integer(YesNoConstant.TYPE_YES));
		}else{
			perfInfo.setBreakdownFlg(new Integer(YesNoConstant.TYPE_NO));
		}
	...
	private static boolean isBreakdown(PerfCheckInfo perfCheckInfo) {
		...
		switch (perfCheckInfo.getBreakdownFlg().intValue()) {
