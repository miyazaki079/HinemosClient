...
public class GetSql {
	 ...
	 * @return SQL監視情報
	 */
	public MonitorInfo getSql(String managerName, String monitorId, int monitorType) {
		...
		try {
			...
			if (monitorType == MonitorTypeMstConstant.MONITOR_TYPE_NUMERIC) {
				info = wrapper.getMonitor(monitorId, HinemosModuleConstant.MONITOR_SQL_N);
			} else {
				info = wrapper.getMonitor(monitorId, HinemosModuleConstant.MONITOR_SQL_S);
			}
