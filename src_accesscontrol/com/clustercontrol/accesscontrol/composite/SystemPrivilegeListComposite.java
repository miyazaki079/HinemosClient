	...
	public void update(String managerName, Object selectedInfo) {
		...
		try {
			if (selectedInfo != null) {
				...
				if (selectedInfo instanceof RoleInfo) {
					if (!((RoleInfo)selectedInfo).getId().equals(RoleSettingTreeConstant.ROOT_ID) &&
						!((RoleInfo)selectedInfo).getId().equals(RoleSettingTreeConstant.MANAGER)) {
						infoList = wrapper.getSystemPrivilegeInfoListByRoleId(((RoleInfo)selectedInfo).getId());
					}
				} else if (selectedInfo instanceof UserInfo) {
					infoList = wrapper.getSystemPrivilegeInfoListByUserId(((UserInfo)selectedInfo).getId());
				}
			}
		...
		if (selectedInfo != null && infoList != null) {
			for (SystemPrivilegeInfo info : infoList) {
				...
				obj.add(SystemPrivilegePropertyUtil.getSystemPrivilegeName(info.getSystemFunction(), info.getSystemPrivilege()));
				obj.add(null);
				listInput.add(obj);
			}
		}
		...
		Object[] args = { new Integer(listInput.size()) };
		if (selectedInfo == null) {
			m_labelType.setText("");
			...
			if (selectedInfo instanceof RoleInfo) {
				if (((RoleInfo)selectedInfo).getId().equals(RoleSettingTreeConstant.ROOT_ID)) {
					...
					m_labelType.setText(Messages.getString("role.name") + " : " + roleInfo.getName() + "(" + roleInfo.getId() + ")");
					m_labelCount.setText(Messages.getString("filtered.records", args));
				}
			} else if (selectedInfo instanceof UserInfo) {
				UserInfo userInfo = (UserInfo)selectedInfo;
				m_labelType.setText(Messages.getString("user.name") + " : " + userInfo.getName() + "(" + userInfo.getId() + ")");
				m_labelCount.setText(Messages.getString("filtered.records", args));
			}
		}
	}
