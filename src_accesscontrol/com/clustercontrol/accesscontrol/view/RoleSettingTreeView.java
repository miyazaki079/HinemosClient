...
public class RoleSettingTreeView extends CommonViewPart {
	...
	public void setEnabledAction(Object selectedInfo, ISelection selection) {
		if(selectedInfo instanceof RoleInfo ) {
			...
			if(RoleSettingTreeConstant.ROOT_ID.equals(roleInfo.getId()) == false &&
					RoleSettingTreeConstant.MANAGER.equals(roleInfo.getId()) == false){
	...
	public static RoleTreeItem getManager(RoleTreeItem item) {
		if (item == null) {
			return null;
				...
				((RoleInfo)item.getData()).getId().equals(RoleSettingTreeConstant.MANAGER)) {
			return item;
		}
