		...
		if (result == null || result.isValid()) {
			if(this.action()){
				try {
					...
					String version = versionBox == null ? null : versionBox.getText();
					...
					NodeSearchResultDialog dialog = new NodeSearchResultDialog(shell, list);
					dialog.open();
					...
					if(existsError) {
						NodeSearchResultErrorDialog errDialog = new NodeSearchResultErrorDialog(shell, list);
						errDialog.open();
					}
					...
					MessageDialog.openError(shell,
							Messages.getString("failed"),
							Messages.getString("message.process.8") + e.getMessage());
				} catch (FacilityDuplicate_Exception e) {
					Object[] arg = { e.getFaultInfo().getFacilityId() };
					MessageDialog.openInformation(shell,
							Messages.getString("message"),
							Messages.getString("message.repository.26", arg) + System.lineSeparator() + e.getMessage());
