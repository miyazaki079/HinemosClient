				...
				for (JobTreeItem mgrTree : JobEditStateUtil.getJobTreeItem().getChildren().get(0).getChildren() ) {
					...
					for (JobTreeItem jobunit : jobEditState.getDeletedJobunitList()) {
						try {
							...
							error = true;
							String jobunitId = jobunit.getData().getJobunitId();
							resultList.put(jobunitId, e.getMessage() + " ("	+ managerName + ")");
						} catch (Exception e) {
							m_log.error("run() delete: " + e.getMessage(), e);
							error = true;
							String jobunitId = jobunit.getData().getJobunitId();
							resultList.put(jobunitId, Messages.getString("message.job.76") + "[" + e.getMessage() + "]" + " (" + managerName + ")");
						}
					}
					...
					for (JobInfo info : jobEditState.getLockedJobunitList()) {
						try {
							...
							error = true;
							resultList.put(info.getJobunitId(), Messages.getString("message.job.80") + "[" + e.getMessage() + "]" + " (" + managerName + ")");
						}
					}
