...
public class ModifyJobAction extends AbstractHandler implements IElementUpdater {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (viewPart instanceof JobListView) {
			...
			JobTreeItem item = view.getSelectJobTreeItemList().get(0);
			if (null == item) {
				return null;
			}
			...
			if (null != parent) {
				...
				JobDialog dialog = new JobDialog(view.getJobTreeComposite(), HandlerUtil.getActiveWorkbenchWindow( event ).getShell(), managerName,
						readOnly);
