...
public class ParameterDialog extends CommonDialog {
	...
	protected void customizeDialog(Composite parent) {
		...
		int type = JobParamTypeConstant.TYPE_SYSTEM_JOB;
		if (m_parameter != null) {
	...
	private void reflectParameter() {
		...
		if (m_parameter != null) {
			...
			if (type == JobParamTypeConstant.TYPE_SYSTEM_JOB ||
				...
				//種別がシステムの場合
				property = new ParameterProperty().getProperty(type);
				ArrayList<Property> propertyList = PropertyUtil.getProperty(property, ParameterProperty.ID_TYPE);
				...
				HashMap<String, Object> map = (HashMap<String, Object>) values[PropertyDefineConstant.SELECT_VALUE][type];
				...
				HashMap<String, Object> map = (HashMap<String, Object>) values[PropertyDefineConstant.SELECT_VALUE][JobParamTypeConstant.TYPE_USER];
	...
	private ValidateResult createParameter() {
		...
		if (type.equals(JobParamTypeConstant.STRING_SYSTEM_JOB)) {
			//パラメータID
			String paramId = "";
			values = PropertyUtil.getPropertyValue(property,
					ParameterProperty.ID_SYSTEM_JOB_PARAM_ID);
			...
			//種別
			m_parameter.add(JobParamTypeConstant.TYPE_SYSTEM_JOB);
		...
		} else if (type.equals(JobParamTypeConstant.STRING_SYSTEM_NODE)) {
			//パラメータID
			String paramId = "";
			values = PropertyUtil.getPropertyValue(property,
					ParameterProperty.ID_SYSTEM_NODE_PARAM_ID);
			if (values.get(0) instanceof String) {
				paramId = (String) values.get(0);
			}
			if (paramId == null || paramId.length() == 0) {
			...
			//種別
			m_parameter.add(JobParamTypeConstant.TYPE_SYSTEM_NODE);
