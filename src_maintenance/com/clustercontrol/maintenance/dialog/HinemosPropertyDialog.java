...
public class HinemosPropertyDialog extends CommonDialog {
	...
	protected void customizeDialog(Composite parent) {
		...
		this.m_valueType.setText(HinemosPropertyTypeConstant.typeToString(this.valueType));
	...
	protected boolean action() {
		...
		if (valueType == HinemosPropertyTypeConstant.TYPE_STRING) {
			...
			if (m_value.getText() == null || m_value.getText().trim().length() == 0) {
				...
				try {
					HinemosPropertyInfo.setValueNumeric(Integer.parseInt(m_value.getText().trim()));
				} catch (NumberFormatException e) {
					m_log.warn("action() setValueNumeric, " + e.getMessage(), e);
					Object[] args = {Messages.getString("hinemos.property.value"), Integer.MIN_VALUE, Integer.MAX_VALUE};
