...
public class CheckInfraManagementAction extends AbstractHandler implements IElementUpdater {
	// ログ
	private static Log m_log = LogFactory.getLog( CheckInfraManagementAction.class );

	/** アクションID */
	public static final String ID = CheckInfraManagementAction.class.getName();
			...
			for(String managementId : entry.getValue()) {
				try {
					...
					Object[] arg = new Object[]{Messages.getString("infra.module.id"), Messages.getString("infra.module.run"),
							Messages.getString("failed"), e1.getMessage()};
		...
		for(Map.Entry<String, List<InfraManagementInfo>> entry : managementMap.entrySet()) {
			...
			for(InfraManagementInfo management : entry.getValue()) {
				List<AccessInfo> accessInfoList = AccessUtil.getAccessInfoList(viewPart.getSite().getShell(), 
						management.getFacilityId(), management.getOwnerRoleId(), managerName, view.isUseNodeProp());
				...
				try {
					...
					m_log.error("execute() :  " + e1.getClass().getName() + ", " + e1.getMessage());
					Object[] arg = new Object[]{Messages.getString("infra.module.id"), Messages.getString("infra.module.run"), Messages.getString("failed"), e1.getMessage()};
					errorMsgs.put(managerName, Messages.getString("message.infra.action.result", arg));
					continue;
				}
			}
