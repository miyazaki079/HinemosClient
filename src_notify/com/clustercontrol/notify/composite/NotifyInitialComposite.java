...
public class NotifyInitialComposite extends Composite {
	...
	public void setInputData(NotifyInfo notify) {
		if(notify != null){
			...
			if (notify.getNotFirstNotify() != null && notify.getNotFirstNotify() == YesNoConstant.TYPE_YES) {
		...
		if(info != null){
			...
			try {
				...
				if (this.m_checkNotRenotifyFirst.getSelection())
					info.setNotFirstNotify(YesNoConstant.TYPE_YES);
				else {
					info.setNotFirstNotify(YesNoConstant.TYPE_NO);
				}
