...
public class NotifyMailCreateDialog extends NotifyBasicCreateDialog {
	...
	private void setInputData(NotifyMailInfo mail) {
		...
		Integer[] validFlgs = getValidFlgs(mail);
	...
	protected NotifyInfo createInputData() {
		...
		// イベント情報
		NotifyMailInfo mail = createInputData(info.getNotifyId());
		info.setNotifyMailInfo(mail);

		return info;
	}

	private NotifyMailInfo createInputData(String notifyId) {
		NotifyMailInfo mail = new NotifyMailInfo();
		mail.setNotifyId(notifyId);

		// メール通知
		mail.setInfoValidFlg(ValidConstant.booleanToType(m_checkMailNormalInfo.getSelection()));
		mail.setWarnValidFlg(ValidConstant.booleanToType(m_checkMailNormalWarning.getSelection()));
		mail.setCriticalValidFlg(ValidConstant.booleanToType(m_checkMailNormalCritical.getSelection()));
		mail.setUnknownValidFlg(ValidConstant.booleanToType(m_checkMailNormalUnknown.getSelection()));
